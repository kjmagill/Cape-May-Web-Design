...
const BlogCard: React.FC<{ post: BlogPost }> = ({ post }) => {
    ...
    return (
        <div className="group bg-slate-800/50 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-700/50 h-full flex flex-col overflow-hidden transition-all duration-500 transform lg:hover:-translate-y-2 lg:hover:border-cyan-500/50 lg:hover:shadow-2xl lg:hover:shadow-cyan-500/10">
            <a href={post.url} target="_blank" rel="noopener noreferrer" className="block overflow-hidden aspect-[16/9] relative bg-slate-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-800">
                ...
                <img 
                    ...
                    loading="lazy"
                    decoding="async"
                    className={`w-full h-full object-cover transform lg:group-hover:scale-105 transition-all duration-700 ${isLoading ? 'opacity-0' : 'opacity-100'} filter brightness-[0.85] saturate-[0.8] lg:group-hover:filter-none`}
                />
            </a>
            <div className="p-8 flex flex-col flex-grow">
                ...
                <a href={post.url} target="_blank" rel="noopener noreferrer" className="text-cyan-400 font-bold inline-flex items-center space-x-2 lg:group-hover:text-cyan-300 transition-all duration-300 focus:outline-none focus-visible:ring-1 focus-visible:ring-cyan-400 rounded-sm">
                    <span className="text-sm uppercase tracking-widest">Read More</span>
                    <ArrowRightIcon className="w-4 h-4 transition-transform duration-300 lg:group-hover:translate-x-2" />
                </a>
            </div>
        </div>
    );
